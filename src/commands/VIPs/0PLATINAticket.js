...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.ManageGuild)) {
        ...
        .setColor("Green")
        .setAuthor({ name: "VIP PLATINA💠", iconURL: interaction.guild.iconURL({ dynamic: true}) })
        .setDescription(`> Clique no botão abaixo para adquirir seu VIP!`)
        .setThumbnail(`https://abs.twimg.com/emoji/v2/72x72/1f4a0.png`)

        let botao = new Discord.ActionRowBuilder().addComponents(
            new Discord.ButtonBuilder()
            .setCustomId("ticket_platina")
            .setEmoji("🎫")
